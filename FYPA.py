...
from altium_annotations import _describe_directive, parse_annotations
from altium_extract import extract_project
from altium_geometry import _save_quicklook, build_layer_geometries
from altium_loader import build_problem, build_solve_metadata, load_project
from lean_solution import LeanSolution, to_lean_solution
...
if getattr(sys, "frozen", False):
    ...
    _LOG_FILE: Path = Path(__file__).parent / "log" / "fypa.log"
...
_SOLVE_TOOL_SOURCES: tuple[Path, ...] = tuple(
    Path(__file__).parent / name for name in (
        "FYPA.py",
        "pdnsolver/problem.py",
        "pdnsolver/mesh.py",
        "pdnsolver/solver.py",
        "lean_solution.py",
    )
...
def _load_split_solve_cache(f) -> dict:
    ...
    from lean_solution import LeanSolution
...
def do_show(args: argparse.Namespace) -> int:
    if not _require_pyside6("show"):
        return 2
    import altium_viewer  # PySide6 import lives inside the viewer
